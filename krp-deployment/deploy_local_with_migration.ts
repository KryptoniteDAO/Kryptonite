...
async function main(): Promise<void> {
  ...
  let hubAddress = await instantiateContract(
    terra,
    test1,
    originalHubCodeId,
    { epoch_period: 300, er_threshold: "1.0", peg_recovery_fee: "0", reward_denom: "uusd", unbonding_period: 6, underlying_coin_denom: "uluna", validator: "terravaloper1dcegyrekltswvyy0xy69ydgxn9x8x32zdy3ua5" },
    new Coins({ uluna: 1000000 })
  );

  let rewardAddress = await instantiateContract(terra, test1, originalRewardCodeId, { hub_contract: hubAddress, reward_denom: "uusd" }, new Coins({}));

  let blunaTokenAddress = await instantiateContract(terra, test1, originalBlunaTokenCodeId, { decimals: 6, hub_contract: hubAddress, initial_balances: [{ address: hubAddress, amount: "1000000" }], name: "bluna", symbol: "BLUNA", mint: { minter: hubAddress, cap: null } }, new Coins({}));
  ...
  await executeContract(
    ...
    hubAddress,
    {
      update_config: { token_contract: blunaTokenAddress, reward_contract: rewardAddress }
    },
    new Coins({})
  );
  ...
  for (let i = 0; i < 3000; i++) {
    ...
    const send = await test1.createAndSignTx({ msgs: [new MsgSend(test1.key.accAddress, wallet.key.accAddress, { uluna: 1000000000, uusd: 1000000000 })] });
    ...
    await executeContract(terra, wallet, blunaTokenAddress, { send: { contract: hubAddress, amount: "100", msg: Buffer.from(JSON.stringify({ unbond: {} })).toString("base64") } }, new Coins({}));
  ...
  let rewardsDispatcherAddress = await instantiateContract(
    terra,
    test1,
    rewardsDispatcherCodeId,
    { lido_fee_address: test1.key.accAddress, lido_fee_rate: "0.05", hub_contract: hubAddress, bluna_reward_contract: rewardAddress, stluna_reward_denom: "uluna", bluna_reward_denom: "uusd" },
    new Coins({})
  );

  let validatorsRegistryAddress = await instantiateContract(terra, test1, validatorsRegistryCodeId, { hub_contract: hubAddress, registry: [{ active: true, address: "terravaloper1dcegyrekltswvyy0xy69ydgxn9x8x32zdy3ua5", total_delegated: "0" }] }, new Coins({}));

  let stlunaTokenAddress = await instantiateContract(terra, test1, stlunaTokenCodeId, { decimals: 6, hub_contract: hubAddress, initial_balances: [], name: "stluna", symbol: "STLUNA", mint: { minter: hubAddress, cap: null } }, new Coins({}));
  ...
  await executeContract(terra, test1, blunaTokenAddress, { send: { contract: hubAddress, amount: "100", msg: Buffer.from(JSON.stringify({ unbond: {} })).toString("base64") } }, new Coins({}));
  ...
  await executeContract(terra, test1, stlunaTokenAddress, { send: { contract: hubAddress, amount: "1000000", msg: Buffer.from(JSON.stringify({ unbond: {} })).toString("base64") } }, new Coins({}));

  await executeContract(terra, test1, blunaTokenAddress, { send: { contract: hubAddress, amount: "1000000", msg: Buffer.from(JSON.stringify({ unbond: {} })).toString("base64") } }, new Coins({}));
}
