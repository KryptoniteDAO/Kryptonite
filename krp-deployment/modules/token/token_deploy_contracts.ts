...
(async (): Promise<void> => {
  ...
  await deployTokenDispatcher(walletData, network);
  await deployTokenTreasure(walletData, network);
  ...
  await deployTokenKeeper(walletData, network, stable_coin_denom);
