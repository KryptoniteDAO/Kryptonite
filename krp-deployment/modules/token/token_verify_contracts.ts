...
async function main(): Promise<void> {
  ...
  if (seilor?.address) {
    ...
    const balanceRes: BalanceResponse = await seilorQueryClient.balance({ address: walletData?.activeWallet?.address });
  ...
  if (veSeilor?.address) {
    ...
    const balanceRes: BalanceResponse = await veSeilorQueryClient.balance({ address: walletData?.activeWallet?.address });
  ...
  if (fund?.address) {
    ...
    const claimAbleSeilorResponse: GetClaimAbleSeilorResponse = await fundQueryClient.getClaimAbleSeilor({ user: walletData?.activeWallet?.address });
    console.log(`\n   Query ${TOKEN_MODULE_NAME}.fund getClaimAbleSeilor ok. \n   ${JSON.stringify(claimAbleSeilorResponse)}`);
    const reservedSeilorForVestingResponse: GetReservedSeilorForVestingResponse = await fundQueryClient.getReservedSeilorForVesting({ user: walletData?.activeWallet?.address });
