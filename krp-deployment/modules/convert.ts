...
import { loadingWalletData, loadingMarketData, loadingStakingData, chainConfigs, STAKING_ARTIFACTS_PATH, MARKET_ARTIFACTS_PATH, CONVERT_ARTIFACTS_PATH, CONVERT_MODULE_NAME } from "../env_data";
...
export function getConvertDeployFileName(chainId: string): string {
  return `deployed_${CONVERT_MODULE_NAME}_${chainId}`;
}

export function convertReadArtifact(networkMarket: ConvertDeployContracts, chainId: string): ConvertDeployContracts {
